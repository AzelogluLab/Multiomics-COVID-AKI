...
# Retrieve genes associated with multiple pathways and combine them
genes1 <- getGeneList("GO:0022610~biological adhesion")
genes2 <- getGeneList("GO:0007155~cell adhesion")
genes3 <- getGeneList("GO:0030155~regulation of cell adhesion")
genes4 <- getGeneList("GO:0098609~cell-cell adhesion")
genes5 <- getGeneList("GO:0031589~cell-substrate adhesion")
genes6 <- getGeneList("GO:0010810~regulation of cell-substrate adhesion")
genes7 <- getGeneList("GO:0034446~substrate adhesion-dependent cell spreading")

# Combine genes from all pathways and remove duplicates
genesToConsider <- c(genes1, genes2, genes3, genes4, genes5, genes6, genes7)
genesToConsider <- unique(genesToConsider)

# Remove specified genes
genesToRemove <- c("CLEC4G")
...
# Sort genes by maximum uniqueness within each cell type
for (j in 1:length(scOrder)) {
...
pdf(file = 'scUnique_Healthy_AdhesionRelated_supervised_nephronorder_withOrganoids.pdf', height = 13, width = 7)
